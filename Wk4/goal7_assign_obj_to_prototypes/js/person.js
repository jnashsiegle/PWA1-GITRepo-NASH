...
window.Person = Person;    //Adding the person object to the global window for main.js
    ...
    Person.jobs = ["Fireman", "Grocer", "Programmer", "Musician"];   //creating jobs for the people
    Person.actions = ["Working Out", "Shopping", "Sleeping", "Eating", "Reading", "Working"];  //activities for the people
    ...
    function Person(name, row){                         //FN to create the person
        ...
        this.job = Person.jobs[Math.floor[Math.random() + Person.actions.length]]; //Persons starting position

        this.action = Person.actions[Math.floor[Math.random() + Person.actions.length]];  //how the person will start
