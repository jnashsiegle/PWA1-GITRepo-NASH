...
// setter
	person["age"] = 40;		//changing person.age value / if index  did not exist it would create it
	person["name"] = "James Bond";	//changing person.name
	console.log(person);   //THIS DOES NOT USE THE PERSON OBJECT FROM ABOVE, ITS A TOTALLY NEW OBJECT


//nested objects   - an object within an object

	person = {birthday: {month: 02, day: 12}, name: "bond"};
	console.log("nest objects " + person);
...
//Window DOM object
	...
	var nav = document.querySelectorAll("#nav li");		//var nav = document.querySelectorAll("#nav li:last-child"); filters down to the last item in array (google)
	console.log(nav);
	...
	console.log(apple);
	console.log((apple.parentNode.parentNode.parentNode).childNodes);  //parentNode childNode siblingNode*/
	/*
	 ==================================================================
	...
	//for(var i = 0, max = navLinks.length; i < max; i++) {
		...
		//if (href === "#1") {										//something here is BROKE getAttribute is resolving to not a function..moving on but making notes
...
//This section utlizes the for loop above in Manipulating href which is why the last curly bracket up there is //'d out
		//and inserted down here

		//var aClass = navLinks[i].firstChild.getAttributes("class");
		//console.log("Manipulating Class Item: ", aClass);
...
var nav1 = document.querySelectorAll('#nav li a');

	/*for (var i = 0, max = nav1.length; i < max; i++) {

		nav1[i].onclick = function(e){
			console.log(e);
			e.preventDefault();
...
*/
	for(var i = 0, max = nav1.length; i < max; i++) {
		nav1[i].onclick = function (e) {
			...
			for (var ii = 0, max2 = nav.length; ii < max2; i++){
...
*/
	...
	nav1[0].setAttribute("class", "navitem active");
